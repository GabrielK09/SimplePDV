...
func HandleProcessGetDashBoardPopularItens(w http.ResponseWriter, r *http.Request) {
	...
	startDate, err := time.ParseInLocation(layout, popularItens.StartDateStr, loc)
	...
	endDate, err := time.ParseInLocation(layout, popularItens.EndDateStr, loc)
	...
	popularItens.StartDate = startDate
	popularItens.EndDate = endDate

	u.GeneralLogger.Printf("Dados para geração do dashboard, inicio: %s - fim %s: ", popularItens.StartDate, popularItens.EndDate)

	popularItensData, err := dashBoard.ShowPopularItens(popularItens.StartDate, popularItens.EndDate)
	...
	resp := responsehelper.Response(true, popularItensData, "Dados do período sucesso!")
